// Service worker: the only place allowed to reach the local analyzer (a content
// script's fetch is blocked by Gmail's CSP/CORS). Content script -> here -> API.
...
async function postJSON(path, body, timeoutMs) {
  ...
  try {
    const res = await fetch(base + path, {
      ...
      signal: ctrl.signal,
      // NO credentials: wildcard CORS forbids cookies.
    });
