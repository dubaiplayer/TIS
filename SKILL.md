...
- SaaS-hosting abuse: phishing increasingly hosts its credential-harvest page on a trusted platform (Google Forms, Microsoft Forms/SharePoint, DocuSign, Typeform, `*.web.app`, etc.) sent from a real mailbox, so link reputation/age/blocklists all read "clean". The `saas_abuse` attribute flags a link to such a host being used as *third-party* credential/login hosting (the link domain differs from the sender's, and the host is an input-collecting form or the email shows login intent). It stays silent on genuine file shares and first-party service mail (e.g. a real DocuSign email from `docusign.net`), and it also blocks the sender-trust discount below so an authenticated sender can't launder it.
- Sender-trust discount: genuine mail whose wording overlaps with phishing (bank alerts, receipts, security notices) is a common false positive. When the raw email includes headers proving it is authenticated (SPF/DKIM/DMARC = pass) AND self-consistent (no link/attachment/obfuscation trickery), the risk is lowered so it isn't flagged on wording alone — a known brand, a `List-Unsubscribe` header, or a **well-established sender domain** (registered years ago, checked via RDAP) clears it; an authenticated but freshly-registered/unknown domain is softened to "suspicious"; and a spoof that fails authentication stays "phishing". Include the full headers when you have them to benefit; body-only text is scored exactly as before.
