...
def analyze(text, sender=None, use_classifier=True, context=None):
    ...
    # Sender-trust discount: if the headers prove the mail is genuinely from its
    # sender (auth pass) and it is self-consistent, LOWER the risk so real bank /
    # transactional mail isn't flagged on shared wording. No-op without headers, so
    # the original rule/ML balance is untouched for body-only text.
    ...
    if tr and overall["risk_score"] > tr["cap"]:
        ...
        if overall["verdict"] == "legitimate":
            overall["top_signals"] = []   # fired attributes are shown, not headlined
